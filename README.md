...
- **디자인 시스템**: `src/theme`에 scale → semantic → app 토큰 3계층, 라이트/다크 테마. 공용 컴포넌트는 온디바이스 Storybook(`pnpm storybook`)으로 문서화
- **커뮤니티 WebView**: `chattea-web`은 Fastify + Vite middlewareMode로 개발 서버를 구성하고, `/community`를 `renderToPipeableStream`으로 스트리밍. 모바일 탭이 WebView로 로드. 서버가 쿼리를 미리 실행해 dehydrated state를 HTML에 넣고, 클라이언트는 `useSuspenseQuery` + `Suspense`/`ErrorBoundary` 경계로 읽는다
- **요청 상관키**: GraphQL 요청마다 `x-request-id`(installId-session-seq)와 `x-device-id`를 헤더로 보내 백엔드 `http_request` 로그와 같은 키로 조인
- **의도 기반 prefetch**: 방 목록 행의 `onPressIn`에서 Apollo `query`로 메시지를 미리 캐시에 채워 진입 대기를 줄인다
